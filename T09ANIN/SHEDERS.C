...
static VOID DG5_RndShaderLog( CHAR *Stage, CHAR *Text )
{
  ...
  if ((F = fopen("DG5{SHAD}30.LOG", "a")) != NULL)
  {
    fprintf(F, "%s: %s\n", Stage, Text);
    fclose(F);
  }
} /* End of 'DG5_RndShaderLog' function */
 ...
 */
static CHAR * DG5_RndShaderLoadTextFile( CHAR *FileName )
{
  FILE *F;
  INT flen;
  CHAR *text;
  ...
  fclose(F);
  return text;
} /* End of 'DG5_RndShaderLoadTextFile' function */
...
UINT DG5_RndShaderLoad( CHAR *FileNamePrefix )
{
  ...
  for (i = 0; isok && i < NS; i++)
  {
    ...
    {
      DG5_RndShaderLog(suff[i], "Error create shader");
      isok = FALSE;
      break;
    }
    /* Load text file */
    sprintf(buf, "%s.%s", FileNamePrefix, suff[i]);
    if ((txt = DG5_RndShaderLoadTextFile(buf)) == NULL)
    {
      DG5_RndShaderLog(suff[i], "No file");
      isok = FALSE;
      break;
    }
  ...
  return prg;
} /* End of 'DG5_RndShaderLoad' function */
...
VOID DG5_RndShaderFree( UINT Prg )
{
  ...
  glDeleteProgram(Prg);
} /* End of 'DG5_RndShaderFree' function */
